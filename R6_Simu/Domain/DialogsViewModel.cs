...
namespace R6_Simu.Domain
{
    public class DialogsViewModel : ViewModelBase
    {
        ...
        #endregion
        public static ICommand ShowSaveSucsseceDiag => new ShowDialogCommand(async () =>
        {
            DelayMessageDialog view = new();
            view.Message.Content = "保存成功！";
            var result = DialogHost.Show(view, "RootDialog");
            await Task.Delay(TimeSpan.FromSeconds(3))
                .ContinueWith((t, _) => DialogHost.CloseDialogCommand.Execute(null, view), null,
                    TaskScheduler.FromCurrentSynchronizationContext());
        });
        public static ICommand ShowModifySucsseceDiag => new ShowDialogCommand(async () =>
        {
            DelayMessageDialog view = new();
            view.Message.Content = "修改成功！";
            var result = DialogHost.Show(view, "RootDialog");
            await Task.Delay(TimeSpan.FromSeconds(3))
                .ContinueWith((t, _) => DialogHost.CloseDialogCommand.Execute(null, view), null,
                    TaskScheduler.FromCurrentSynchronizationContext());
        });
        public static ICommand ShowErrorDiag => new ShowDialogCommand(async () =>
        {
            DelayMessageDialog view = new();
            view.Message.Content = "出现错误！";
            var result = DialogHost.Show(view, "RootDialog");
            await Task.Delay(TimeSpan.FromSeconds(3))
                .ContinueWith((t, _) => DialogHost.CloseDialogCommand.Execute(null, view), null,
                    TaskScheduler.FromCurrentSynchronizationContext());
        ...
        static bool d = true;
        public static ICommand HelpDialogCommand => new AnotherCommandImplementation(async (e) => {
            var view = new DelayMessageDialog();
            view.Message.Content = d ? "我们每日的奋斗是修筑明日眺望世界的阶梯" : "不积跬步，无以至千里; 不积小流，无以成江海";
            d = !d;
            //show the dialog
            var result = DialogHost.Show(view, "RootDialog");
            await Task.Delay(TimeSpan.FromSeconds(3))
